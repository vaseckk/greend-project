...
function NewTaskSubtask(): JSX.Element {
    ...
    dispatch(createTask(EpicData))
      .then((action) => {
        if (createTask.fulfilled.match(action)) {
          const path = generatePath(`${AppRoute.Task}`, { id: action.payload.simpleId });
          navigate(path);
        }
      });
  };
